...
from util import sample_and_group_continuous
...
class Pct(nn.Module):
    ...
    def forward(self, point_list, train):
        if train==True:    
            batch_size, _, _ = point_list[0].size()
            representation_list = []
            Loss_feat_list = []
            for i in range(len(point_list)):   
                ...
                # print(x.shape)
                new_feature = sample_and_group_continuous(npoint=256, nsample=16, xyz=xyz, points=x)    
                # print(new_xyz.shape, new_feature.shape)
                feature_0 = self.gather_local_0(new_feature)
                feature = feature_0.permute(0, 2, 1)
                new_feature = sample_and_group_continuous(npoint=256, nsample=16, xyz=xyz, points=feature) 
                ...
                x = self.conv_fuse(x)
                Loss_feature = F.adaptive_max_pool1d(x, 1).view(batch_size, -1)
                representation_list.append(x)
                Loss_feat_list.append(Loss_feature)
            return representation_list, Loss_feat_list
