...
class TrainerIteration(object):
    ...
    def make_network_input(self):
        ...
        :return:
        """
        # print("run_single_eval2:", self.opt.run_single_eval)
        if self.flags.train:  
            ...
            # fps_idx = pointnet2_utils.furthest_point_sample(self.data.target, 128).long() # [B, npoint]
            # self.data.network_input = index_points(self.data.target, fps_idx).transpose(2, 1)
        else:
            ################################### PUGAN ##########################################
            ...
            # ball_idx = pointnet2_utils.ball_query(2.5, 256, self.data.network_input, seed_xyz).long() # [B, npoint, nsample]
    ...
    def common_ops(self):
        ...
        # print(self.data.network_input.size(0))
        if self.flags.train:    
            ...
            # self.data.pointsReconstructed_prims = torch.cat(self.outpatch_list, dim=1).transpose(1,2)  # 1 3 pts
        #############################################################################
