...
from training.trainer_iteration_PU1K import TrainerIteration
...
from KITTI_to_pts import load_calib
...
class Trainer(TrainerAbstract, TrainerLoss, TrainerIteration, TrainerDataset, TrainerModel):
    def __init__(self, opt):
        ...
        if self.opt.run_single_eval ==True:
            self.writer = SummaryWriter('./val_board')
        else:
            self.writer = SummaryWriter(self.opt.tensorboard_dir)
        self.K = load_calib()
    ...
    def train_loop(self):
        ...
        for data in iterator:
            self.increment_total_iteration()
            ...
            # if self.total_iteration % 4000==0:
            #     self.save_iteration_network(self.total_iteration)
