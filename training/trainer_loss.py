...
class TrainerLoss(object):
    ...
    def chamfer_loss(self):
        ...
        if not self.flags.train:
            ...
            dist1, dist2, idx1, idx2 = self.distChamfer(inCham1, inCham2)
            self.data.loss_fscore, _, _ = fscore(dist1, dist2)
            self.data.loss_fscore = self.data.loss_fscore.mean()
            self.data.loss = torch.mean(dist1) + torch.mean(dist2)
            self.data.hausdorff = (torch.max(dist1,dim=1)[0] + torch.max(dist2,dim=1)[0])
            
        if self.flags.train:
            inCham_target = self.data.target.contiguous()
            inCham0 = self.data.final_list[0].contiguous()
            inCham1 = self.data.final_list[1].contiguous()
            inCham2 = self.data.final_list[2].contiguous()
            inCham3 = self.data.final_list[3].contiguous()
            # print(inCham_target.shape)
            # print(inCham0.shape)
            # print(inCham1.shape)
            # print(inCham2.shape)
            # print(inCham3.shape)
            
            dist1, dist2, idx1, idx2 = self.distChamfer(inCham_target, inCham0)
            dist1_1, dist1_2, idx1, idx2 = self.distChamfer(inCham_target, inCham1)  # mean over points
            dist2_1, dist2_2, idx1, idx2 = self.distChamfer(inCham_target, inCham2)  # mean over points
            dist3_1, dist3_2, idx1, idx2 = self.distChamfer(inCham_target, inCham3)  # mean over points
            
            self.data.loss0 = torch.mean(dist1)+torch.mean(dist2)
            self.data.loss1 = torch.mean(dist1_1)+torch.mean(dist1_2)
            self.data.loss2 = torch.mean(dist2_1)+torch.mean(dist2_2)
            self.data.loss3 = torch.mean(dist3_1)+torch.mean(dist3_2)
            
            self.data.smooth1 = self.SmoothL1Loss(self.data.feat_list[0], self.data.feat_list[1])
            # print(self.data.smooth1)
            self.data.smooth2 = self.SmoothL1Loss(self.data.feat_list[0], self.data.feat_list[2])
            # print(self.data.smooth2)
            self.data.smooth3 = self.SmoothL1Loss(self.data.feat_list[0], self.data.feat_list[3])
            # print(self.data.smooth3)
            self.data.smooth_loss = self.data.smooth1 + self.data.smooth2 + self.data.smooth3
            
            self.data.cham_loss = self.data.loss0 + self.data.loss1 + self.data.loss2*0.5 + self.data.loss3*0.1
            # self.data.loss = self.data.loss0
            self.data.loss = self.data.cham_loss + self.data.smooth_loss*0.1
            self.writer.add_scalar("loss_total",self.data.loss, self.total_iteration)
            self.writer.add_scalar("chamfer_loss_total",self.data.cham_loss, self.total_iteration)
            self.writer.add_scalar("chamfer_loss0",self.data.loss0, self.total_iteration)
            self.writer.add_scalar("chamfer_loss1",self.data.loss1, self.total_iteration)
            self.writer.add_scalar("chamfer_loss2",self.data.loss2, self.total_iteration)
            self.writer.add_scalar("chamfer_loss3",self.data.loss3, self.total_iteration)
            self.writer.add_scalar("smooth_loss",self.data.smooth_loss, self.total_iteration)
        # else:
        #     self.data.loss = self.data.loss0
