...
class TrainerAbstract(object):
    ...
    def save_epoch_network(self, epoch):
        print("saving net for each epoch...")
        self.network_epoch_pth = join(self.opt.dir_name, str(epoch)+"_network.pth")
        self.optimizer_epoch_pth = join(self.opt.dir_name, str(epoch)+ "_optimizer.pth")
        torch.save(self.network.state_dict(), self.network_epoch_pth)
        torch.save(self.optimizer.state_dict(),self. optimizer_epoch_pth)
        print("network saved for each epoch")
        
    def save_iteration_network(self, iteration):
        print("saving net for iteration...")
        self.network_iteration_pth = join(self.opt.dir_name, str(iteration)+"_network.pth")
        self.optimizer_iteration_pth = join(self.opt.dir_name, str(iteration)+ "_optimizer.pth")
        torch.save(self.network.state_dict(), self.network_iteration_pth)
        torch.save(self.optimizer.state_dict(),self. optimizer_iteration_pth)
        print("network saved for iteration")
    ...
    def print_iteration_stats(self, loss, loss_cham, loss_smooth):
        """
        ...
        print(
            ...
            + "] cham loss:  "
            + colored(f"{loss_cham.item()} ", "cyan")
            + "] smooth loss:  "
            + colored(f"{loss_smooth.item()} ", "red"),
    ...
    def learning_rate_scheduler(self):
        ...
        if self.epoch == self.opt.lr_decay_3:
            ...
            self.optimizer = optim.Adam(self.network.parameters(), lr=self.opt.lrate)
        if self.epoch == self.opt.lr_decay_4:
            self.opt.lrate = self.opt.lrate / 10.0
            print(f"Third learning rate decay {self.opt.lrate}")
            self.optimizer = optim.Adam(self.network.parameters(), lr=self.opt.lrate)
    ...
    def reset_total_iteration(self):
